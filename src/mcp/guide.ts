...
Routine authorized changes apply directly and are audited. planner.history returns bounded summaries and only the latest eligible transaction can be reversed with planner.undo; intervening browser or agent edits prevent undo. MCP v1 cannot delete accounts, administer credentials or connections, or replace all planner data. Destructive subordinate operations are not exposed in the initial tool surface.`;
...
5. Inspect \`planner.history\` and use \`planner.undo\` for the latest eligible transaction when recovery is needed. Intervening edits prevent undo; use a new command batch to revise older changes.
