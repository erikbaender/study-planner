...
describe("AccountSettings", () => {
  it("requests and verifies a new email through separate backend calls", async () => {
    ...
    render(<PlannerAuthProvider value={{ status: "authenticated", account: { name: "Ada", email: "old@example.com", image: null }, signIn, signOut: vi.fn() }}><AccountSettings account={{ name: "Ada", email: "old@example.com", image: null }} onSignOut={vi.fn()} /></PlannerAuthProvider>);
    ...
    await user.click(screen.getByRole("button", { name: "Send change code" }));
