...
Supersedes: the UX sections of `REQUIREMENTS.md`
...
1. **Removing Ionic** contradicts the original brief ("Use ... Ionic for the frontend"). It is
   used for one modal, and it is an iOS-mobile toolkit in a macOS-desktop app. **Recommend
   removing.**
2. **Today, not the Gantt, becomes the landing view.** The brief centres the Gantt chart. It
   remains a core view and gets the most engineering effort — but it is a poor answer to
   *"what do I study today?"*. **Recommend Today as the default.**
3. **`REQUIREMENTS.md` is kept but superseded.** You said you were unhappy with how you specified
   the UX. It is your original brief and a useful record, and you did not explicitly ask for it
   to go, so it stays for now. **Say the word and it goes.**
4. **Units are per-topic, not global.** Lena can mix slides, pages and videos across topics.
   Costs a little UI complexity; avoids forcing a fake common unit. **Recommend per-topic.**
